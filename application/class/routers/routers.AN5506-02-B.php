...
class AN550602B
{
	...
	public function Login($Host,$User,$Pass)
	{
		...
		curl_setopt($this->ch, CURLOPT_POSTFIELDS, "User=admin&Passwd=teste");
		...
		echo $result;
		$Status = (strpos($result,"HTTP/1.0 200 OK") !== false) ? true : false;
		$Status2 = (strpos($result,"/login.html") == false) ? true : false;
		if ($Status && $Status2)
		{
			$this->Host = $Host;
			$this->User = $User;
